...
public class HttpsProvisioningManager {
    ...
    private HostnameVerifier mHostnameVerifier = new HostnameVerifier() {
            ...
            public boolean verify(String hostname, SSLSession session) {
                if (hostname != null) {
                    Pattern pattern =
                        Pattern.compile("^config\\.rcs\\.mnc\\d+.mcc\\d+\\.pub\\.3gppnetwork\\.org$");
        ...
        try {
            ...
            if (networkInfo == null) {
                if (sLogger.isActivated()) {
                    sLogger.debug("networkInfo is null");
                }
                return false;
            }
    ...
    private HttpURLConnection executeHttpRequest(boolean secured, String request)
        ...
        if (RcsSettings.ATT_SUPPORT) {
            ...
            if (secured && DeviceUtils.isTestSim()) {
                sLogger.info("secure & test sim");
                NetworkInfo nwInfo = mNetworkCnx.getConnectionMngr().getActiveNetworkInfo();
                if (nwInfo != null && nwInfo.getType() == ConnectivityManager.TYPE_MOBILE) {
                    ((HttpsURLConnection)cnx).setHostnameVerifier(mHostnameVerifier);
                }
            }
        }
    ...
    private HttpsProvisioningResult sendFirstRequestsToRequireOTP(ContactId msisdn,
        ...
        try {
            ...
            result.rspMessage = urlConnection.getResponseMessage();
            ...
            sLogger.info("[sendFirstRequestsToRequireOTP] request result.code:" + result.code);
    ...
    /* package private */void updateConfigWithOTP(String otp, String requestUri)
        ...
        try {
            ...
            result = sendSecondHttpsRequestWithOTP(otp, requestUri);
            ...
            if (RcsSettings.ATT_SUPPORT) {
                ...
                if (result != null
                    && result.code != HttpsProvisioningResult.UNKNOWN_REASON_CODE
                    && result.code != HttpURLConnection.HTTP_OK) {
                    AcsService.onErrorReceived(result.code,
                        (result.rspMessage !=  null) ? result.rspMessage : "");
                } else {
                    int version = mRcsSettings.getProvisioningVersion();
                    AcsService.onConfigChange(AcsConfigInfo.CONFIGURED, version);
                }
            }
        }
    ...
    private HttpsProvisioningResult getConfig() throws IOException {
        ...
        try {
            ...
            String token = mRcsSettings.getProvisioningToken();
            String request = requestUri + getHttpsRequestArguments("37273", token, contactId);
            urlConnection = executeHttpRequest(true, request);
            result.code = urlConnection.getResponseCode();
            result.rspMessage = urlConnection.getResponseMessage();
            if (HttpURLConnection.HTTP_OK != result.code && !StringUtils.isEmpty(secondaryUri)) {
                ...
                request = requestUri + getHttpsRequestArguments("37273", token, contactId);
                urlConnection = executeHttpRequest(true, request);
                result.code = urlConnection.getResponseCode();
                result.rspMessage = urlConnection.getResponseMessage();
            }
                ...
                if (result == null || !result.waitingForSMSOTP) {
                    ...
                    if (result != null
                        ...
                        if ((preVersion != version)
                            || "3".equals(versionCode)
                            || "5".equals(versionCode)) {
                            AcsService.onConfigChange(AcsConfigInfo.CONFIGURED, version);
                        }
                    }
    ...
    /* package private */HttpsProvisioningResult sendSecondHttpsRequestWithOTP(String otp,
        ...
        try {
            ...
            result.code = urlConnection.getResponseCode();
            result.rspMessage = urlConnection.getResponseMessage();
            sLogger.info("[sendSecondHttpsRequestWithOTP] request result.code:" + result.code);
            switch (result.code) {
                case HttpURLConnection.HTTP_OK:
                    result.content = readStream(urlConnection.getInputStream());
                    return result;
    ...
    private void processProvisioningResult(HttpsProvisioningResult result) {
        ...
        if (HttpURLConnection.HTTP_OK == result.code) {
            ...
            try {
                ...
                mRcsSettings.saveProvisioningValidity(validity);
                String versionCode = LauncherUtils.getVersionCode(mCtx);
                switch (versionCode) {
                    // 2: for 19.1
                    case "2":
    ...
    public void provisioningFails(int reason) {
        ...
        if (NetworkUtils.getNetworkAccessType(mCtx) == NetworkUtils.NETWORK_ACCESS_WIFI) {
            sLogger.info("provisioningFails reason:" + reason);
