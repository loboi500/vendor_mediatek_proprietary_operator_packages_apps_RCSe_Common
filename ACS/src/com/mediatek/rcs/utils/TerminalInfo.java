...
public class TerminalInfo {
    ...
    public static String getClientVersion(Context ctx) {
        ...
        String clientVersion = settings.getClientVersion();
        if (clientVersion == null) {
            clientVersion = "RCSAndrd-1.0"; //CLIENT_VERSION_PREFIX + getProductVersion(ctx);
        }
        return clientVersion.substring(0, Math.min(clientVersion.length(), 15));
    }
    ...
    public static String getClientVendor() {
        RcsSettings settings = RcsSettings.getInstance();
        String vendor = settings.getClientVendor();
        if (vendor == null) {
            vendor = "Goog"; //(Build.MANUFACTURER != null) ? Build.MANUFACTURER : UNKNOWN;
        }
    ...
    public static String getTerminalModel() {
        String model = (Build.DEVICE != null) ? Build.MODEL : UNKNOWN;
    ...
    public static String getTerminalSoftwareVersion() {
        String swVersion = (Build.DISPLAY != null) ? Build.VERSION.INCREMENTAL : UNKNOWN;
