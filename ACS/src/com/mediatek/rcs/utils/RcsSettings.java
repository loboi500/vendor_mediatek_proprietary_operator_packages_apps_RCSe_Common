...
public class RcsSettings {
    ...
    private static final String CONFIG_RCS_PROFILE = "rcs_profile";
    private static final String DEFAULT_CONFIG_RCS_PROFILE = "UP_2.3";

    private static final String CONFIG_RCS_VERSION = "rcs_version";
    private static final String DEFAULT_CONFIG_RCS_VERSION = "9.0";
    ...
    static final Long DEFAULT_CAPABILITY_EXPIRY_TIMEOUT = 86400L;
    ...
    static final String LAST_SUB_ID = "LastSubId";
    static final Integer DEFAULT_LAST_SUB_ID = -1;

    private static final Logger sLogger = Logger.getLogger(RcsSettings.class.getName());
    ...
    public boolean readBoolean(String key) {
        ...
        if (value == null) {
            try {
                ...
                sLogger.error("readBoolean error!" + e);
                value = false;
            }
        }
        return value;
    ...
    public int readInteger(String key) {
        ...
        if (value == null) {
            try {
                ...
                sLogger.error("readInteger error!" + e);
                value = -1;
            }
        }
        return value;
    ...
    public long readLong(String key) {
        ...
        if (value == null) {
            try {
                ...
                sLogger.error("readLong error!" + e);
                value = -1L;
            }
        }
        return value;
    ...
    private String readParameter(String key) {
        ...
        try {
            ...
            sLogger.error("readParameter error!" + e);
        } finally {
        ...
        } else if (LauncherUtils.REGISTRY_PROVISIONING_VALIDITY.equals(key)) {
            defaultValue = String.valueOf(LauncherUtils.DEFAULT_PROVISIONING_VALIDITY);
        } else if (LauncherUtils.REGISTRY_PROVISIONING_EXPIRATION.equals(key)) {
            defaultValue = "0";
        } else if (LAST_SUB_ID.equals(key)) {
            defaultValue = String.valueOf(DEFAULT_LAST_SUB_ID);
        }
    ...
    private boolean writeParameter(String key, String value) {
        ...
        try {
            ...
            sLogger.error("writeParameter error!" + e);
        } finally {
            StrictMode.setThreadPolicy(oldPolicy);
        }
        if (!result && sLogger.isActivated()) {
            sLogger.warn("writeParameter key:" + key + ", result:" + result);
        }
        return result;
    }
    ...
    public String getRcsProfile() {
        return readString(CONFIG_RCS_PROFILE);
    }

    public void setRcsProfile(String profile) {
        if (profile != null) {
            writeString(CONFIG_RCS_PROFILE, profile);
        }
    }

    public String getRcsVersion() {
        return readString(CONFIG_RCS_VERSION);
    }

    public void setRcsVersion(String version) {
        if (version != null) {
            writeString(CONFIG_RCS_VERSION, version);
        }
    }
    ...
    public String getLastUserAccount() {
        return readString(LauncherUtils.REGISTRY_LAST_USER_ACCOUNT);
    }

    public void setLastUserAccount(String value) {
        writeString(LauncherUtils.REGISTRY_LAST_USER_ACCOUNT, value);
    }
    ...
    public long getProvisioningExpirationDate() {
        return readLong(LauncherUtils.REGISTRY_PROVISIONING_EXPIRATION);
    }
     ...
     */
    public long getProvisioningValidity() {
        return readLong(LauncherUtils.REGISTRY_PROVISIONING_VALIDITY);
    }
     ...
     */
    public void saveProvisioningValidity(long validity) {
        // if (validity <= 0L) {
        //     return;
        // }
        /* Calculate next expiration time in milliseconds */
        long next = System.currentTimeMillis();
        if (validity > 0) next += validity;
        writeLong(LauncherUtils.REGISTRY_PROVISIONING_VALIDITY, validity);
        writeLong(LauncherUtils.REGISTRY_PROVISIONING_EXPIRATION, next);
    }

    public int getLastSubId() {
        return readInteger(LAST_SUB_ID);
    }

    public void setLastSubId(int value) {
        writeInteger(LAST_SUB_ID, value);
    }
