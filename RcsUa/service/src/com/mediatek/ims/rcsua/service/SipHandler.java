...
public class SipHandler {
    ...
    void sendMessageAsync(byte[] message, String transactionId, int channelId) {

        if (processor != null) {
            processor.handleSipSendingAsync(message, transactionId, channelId);
        }
    }
    ...
    private void notifySipMessageSent(String transactionId) {
        for (Listener listener : listeners) {
            listener.messageSent(transactionId);
        }
    }

    private void notifySipMessageSendFailure(String transactionId, int reason) {
        for (Listener listener : listeners) {
            listener.messageSendFailure(transactionId, reason);
        }
    }
    ...
    class Processor extends Handler {
        ...
        void handleSipSendingAsync(byte[] msgData, String transactionId, int channelId) {
            post(new Runnable() {
                @Override
                public void run() {
                    ...
                    if (sip != null) {
                        if (!transactionId.equals(sip.branchId)) {
                            logger.error("Inconsistent branchId found, ignore sending");
                            notifySipMessageSendFailure(transactionId, 4);
                            return;
                        }
                        if (sip.isRequest()) {
                        ...
                        notifySipMessageSendFailure(transactionId, 0);
                    }
                }
            });
        }
    ...
    public static abstract class Listener {
        ...
        public abstract void messageSent(String transactionId);
        public abstract void messageSendFailure(String transactionId, int reason);
...
*/
    ...
    private static class Line {
        ...
        int getMethod() {
            ...
            if (startsWith(METHOD_UPDATE))
                method = SipMessage.UPDATE;
