...
public class RuaAdapter {
    ...
    private int mMobileRegRat = TelephonyManager.NETWORK_TYPE_UNKNOWN;
    private long mCid = -1;
    private int mLac = -1;
    ...
    private RcsRIL mRil;
    private int mVops = -1;
    private HashSet<AvailabilityListener> listeners = new HashSet<AvailabilityListener>();
    ...
    public void registerForAvailabilityChange(AvailabilityListener listener) {
        listeners.add(listener);
    }

    public void unregisterForAvailabilityChange(AvailabilityListener listener) {
    ...
    RegistrationInfo getRegistrationInfo() {
        ...
        if (radioTech == RcsUaService.REG_RADIO_IEEE802) {
            if (mLastOosTime == -1) {
                ...
                mLastCid = Long.parseLong(readParameter("KEY_CID", "0"));
            ...
            if (pLANI != null && !pLANI.isEmpty()) {
                if ((mMobileRegRat == TelephonyManager.NETWORK_TYPE_UNKNOWN)
                    && (mLastRegRat != TelephonyManager.NETWORK_TYPE_UNKNOWN)) {
                    try {
                        pLANI += SEMICOLON + "\""
    ...
    private RuaAdapter() {
        ...
        sipHandler = SipHandler.createInstance(this);
         ...
         /*Need to move this code to RuaService, as due to roiSupport being removed,
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_UA_CONNECTED:
                    ...
                    uaConnected = true;
                    if (RoiHandler.getInstance() != null) {
                        RoiHandler.getInstance().processRegistration(
                                new RoiHandler.InputEvent.Builder(RoiHandler.InputType.UA_HAL_CONNECTION)
                                        .setHalConnected(true)
                                        .build());
                    }
                    notifyAvailabilityChanged();
                    break;
                ...
                case MSG_STORE_PLANI:
                    if (mLastRegRat > TelephonyManager.NETWORK_TYPE_UNKNOWN) {
    ...
    public void sendSipMessage(byte[] message, String transactionId, int id) {
        sipHandler.sendMessageAsync(message, transactionId, id);
    }
    ...
    void handleRegistrationInfo(RcsUaEvent event) {
        logger.debug("handleRegistrationInfo event : "+event.toString());
    ...
    private boolean updateRegistrationInfo(RcsUaEvent incomingEvent) {
        ...
        logger.debug("updateRegistrationInfo incomingOwner: "+incomingOwner+" ,incomingImsState: "+incomingImsState+" ,rcsState: "+rcsState+" ,rcsTag: "+rcsTag);
        logger.debug("updateRegistrationInfo regInfo.getRegState(): "+regInfo.getRegState()+" ,regInfo.getRadioTech(): "+regInfo.getRadioTech()+" ,regInfo.getRegMode()(): "+regInfo.getRegMode());
        if (incomingImsState == IMS_REG_STATE_REGISTERED) {
            ...
            imsConfig.setPortS(incomingEvent.getInt());
            imsConfig.setViaPort(incomingEvent.getInt());
            imsConfig.setSecurityVerify(incomingEvent.getString(256));
        ...
        if (REG_MODE_INTERNET == regInfo.getRegMode()) {
            if (incomingOwner == RoiHandler.REG_OWNER_RCS) {
                logger.debug("regInfo.getRegState() : "+regInfo.getRegState());
                switch (regInfo.getRegState()) {
                    case REG_STATE_NOT_REGISTERED:
                    ...
                    case REG_STATE_DEREGISTERING:
                        ...
                        if (incomingImsState == IMS_REG_STATE_REGISTERED) {
                            ...
                            logger.debug("REG_STATE_DEREGISTERING incomingImsState == IMS_REG_STATE_UNREGISTERED");
                            regInfo.setRegState(REG_STATE_NOT_REGISTERED);
                            regInfo.stateUpdated = true;
                        } else if (incomingImsState == IMS_REG_STATE_REGISTERING) {
                            regInfo.setRegState(REG_STATE_REGISTERING);
    ...
    public void sendNetworkRtSettings(int acct_id, String networkInterface,
        long networkid, String localAddress, String[] pCSCFList,
        ...
        event.putInt(RcsEventSettingTLV.VoLTE_Setting_Net_Network_Id);
        byteParam = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN)
                .putLong(networkid).array();
        event.putInt(byteParam.length);
        event.putNBytes(byteParam, byteParam.length);
    ...
    public void sendForceRegReregister(Capability capability) {
        // Use AT command instead of UA command
        logger.debug("sendForceRegReregister capability: "+capability);
        long features = capability.toNumeric();
        Utils.sendAtCommand("AT+EIMSRCS=5," + features);
    }
        ...
        if (mPhoneStateListener == null) {
            mPhoneStateListener = new PhoneStateListener() {
                    ...
                    public void onServiceStateChanged(ServiceState serviceState) {
                        final NetworkRegistrationInfo csRegInfo = serviceState.getNetworkRegistrationInfo(
                                NetworkRegistrationInfo.DOMAIN_CS, AccessNetworkConstants.TRANSPORT_TYPE_WWAN);
                        final NetworkRegistrationInfo iwlanRegInfo = serviceState.getNetworkRegistrationInfo(
                                NetworkRegistrationInfo.DOMAIN_PS, AccessNetworkConstants.TRANSPORT_TYPE_WLAN);
                        final NetworkRegistrationInfo wwanRegInfo = serviceState.getNetworkRegistrationInfo(
                                NetworkRegistrationInfo.DOMAIN_PS, AccessNetworkConstants.TRANSPORT_TYPE_WWAN);
                        int curRat;
                        CellIdentity ci = null;
                        boolean psInService = false;
                        if (iwlanRegInfo != null && iwlanRegInfo.isInService()) {
                            curRat = iwlanRegInfo.getAccessNetworkTechnology();
                            psInService = true;
                        } else if (wwanRegInfo != null && wwanRegInfo.isInService()) {
                            curRat = wwanRegInfo.getAccessNetworkTechnology();
                            ci = wwanRegInfo.getCellIdentity();
                            psInService = true;
                        } else if (csRegInfo != null && csRegInfo.isInService()) {
                            curRat = csRegInfo.getAccessNetworkTechnology();
                            ci = csRegInfo.getCellIdentity();
                        } else {
                            curRat = TelephonyManager.NETWORK_TYPE_UNKNOWN;
                        }

                        if (psInService && isRoiCapable() && RoiHandler.getInstance() != null) {
                            RoiHandler.getInstance().notifyNetworkChanged(curRat);
                        }

                        int lac = 0;
                        long cid = 0;
                        if (ci != null) {
                            mPlmn = ci.getMccString() + ci.getMncString();
                            switch (ci.getType()) {
                                case CellInfo.TYPE_LTE: {
                                    CellIdentityLte lteCi = (CellIdentityLte)ci;
                                    lac = lteCi.getTac();
                                    cid = lteCi.getCi();
                                }
                                    break;
                                ...
                                case CellInfo.TYPE_NR: {
                                    CellIdentityNr nrCi = (CellIdentityNr)ci;
                                    lac = nrCi.getTac();
                                    cid = nrCi.getNci();
                                }
                                    break;
                                ...
                                case CellInfo.TYPE_WCDMA: {
                                    CellIdentityWcdma wCi = (CellIdentityWcdma)ci;
                                    lac = wCi.getLac();
                                    cid = wCi.getCid();
                                }
                                    break;
                                ...
                                case CellInfo.TYPE_GSM: {
                                    CellIdentityGsm gCi = (CellIdentityGsm)ci;
                                    lac = gCi.getLac();
                                    cid = gCi.getCid();
                                }
                                    break;
                        ...
                        if ((lac != 0 && cid != 0)
                            ...
                            mLac = lac;
                            mCid = cid;
                            if (isRoiCapable() && RoiHandler.getInstance() != null) {
                                RoiHandler.getInstance().notifyCellLocationChanged(mLac, mCid);
                            }
                        }
                        if (mMobileRegRat != curRat && curRat != TelephonyManager.NETWORK_TYPE_IWLAN) {
                            if (logger.isActivated()) {
                                logger.debug("onServiceStateChanged: "
                                        + "from [" + mMobileRegRat + "] "
                                        + "to [" + curRat + "]");
                            }
                            if (curRat == TelephonyManager.NETWORK_TYPE_UNKNOWN)
                                storeRegInfo();
    ...
    private boolean writeParameter(String key, String value) {
        ...
        if (manager != null && !manager.isUserUnlocked()) {
            logger.info("user locked, just skip");
            return false;
        }
    ...
    private String readParameter(String key, String defaultValue) {
        ...
        UserManager manager = UserManager.get(context);
        if (manager != null && !manager.isUserUnlocked()) {
            logger.info("user locked, just return default");
            return defaultValue;
        }
    ...
    private String getPLANI(int regRat, int lac, long cid, String plmn) {
    ...
    public void notifyVopsIndication(int vops) {
        boolean roiVops = true;
        if (isRoiCapable()) {
            RoiHandler roiHandler = RoiHandler.getInstance();
            if (roiHandler != null) {
                roiVops = roiHandler.getCurrentVops();
                logger.debug("roi handler VOPs value : " + roiVops);
            } else {
                logger.debug("roi handler is null to check VOPs value");
            }
        }

        if (mVops == vops && roiVops) {
