...
public class RoiHandler {
    ...
    private final boolean UT_SUPPORT = false;
    ...
    private boolean mUaHalConnected = false;
    ...
    public static class InputEvent {
        ...
        public String toString() {
            switch (type) {
                ...
                case UA_HAL_CONNECTION:
                    return type.toString() + ": " + "hal[" + halConnectd + "]";
        ...
        public static class Builder {
            ...
            public Builder setHalConnected(boolean halConnected) {
                this.halConnected = halConnected;
                return this;
            }
            ...
            public InputEvent build() {
                ...
                input.halConnectd = halConnected;
    ...
    public static synchronized void deleteInstance() {
        if (sInstance != null) {
            ...
            if (sInstance.mImsFeatureReceiver != null) {
                mContext.unregisterReceiver(sInstance.mImsFeatureReceiver);
            }
    ...
    private RoiHandler() {
        ...
        mUaHalConnected = adapter.isHalConnected();
        ...
        logger.debug("mIsVolteEnabled: " + mIsVolteEnabled);

        mIsVowifiEnabled = mImsManager.isWfcEnabledByUser();
        logger.debug("mIsVowifiEnabled: " + mIsVowifiEnabled);

        IntentFilter filter = new IntentFilter();
        filter.addAction(ImsConfig.ACTION_IMS_FEATURE_CHANGED);
        mContext.registerReceiver(mImsFeatureReceiver, filter);
    ...
    /* Check the current VOPs value */
    public boolean getCurrentVops(){
         logger.debug("current VoPS: " + mVoPS);
         return mVoPS;
    }
    ...
    public void notifyCellLocationChanged(int lac, long cid) {
        mHelper.mLac = Integer.toHexString(lac);
        mHelper.mCid = Long.toHexString(cid);
    }

    public void notifyNetworkChanged(int networkType) {
            ...
            public void handleMessage(Message msg) {
                ...
                switch(msg.what) {
                    ...
                    case MSG_DO_IMS_AUTH:
                        ...
                        int auth_result = 1;
                        byte[] res = new byte[0];
                        byte[] auts = new byte[0];
                        String ckEsp = null;
                        String ikEsp = null;
                        if (resp != null) {
                            auth_result = resp.isAuthSuccessful()? 0:1;
                            res = resp.getRES();
                            auts = resp.getAuts();
                            ckEsp = resp.getCkEsp();
                            ikEsp = resp.getIkEsp();
                        ...
                        handle401Unauthorized("00000000", auth_result, res,
                                ckEsp, ikEsp, auts, userdata);
                        break;
    ...
    public synchronized void processRegistration(InputEvent input) {
        ...
        switch (input.getType()) {
            ...
            case UA_HAL_CONNECTION:
                if (mUaHalConnected != input.isHalConnectd()) {
                    mUaHalConnected = input.isHalConnectd();
                    halChanged = true;
                }
                break;
        ...
        if (!imsRegChanged && !networkChanged && !vopsChanged
                ...
                && !callChanged && !timeoutChanged && !halChanged) {
        ...
        switch (mRegState) {
            ...
            case IDLE:
                if (InputType.INTERNET_PDN_CHANGED == input.getType()
                        ...
                        || InputType.UA_HAL_CONNECTION == input.getType()) {
    ...
    private BroadcastReceiver mImsFeatureReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ImsConfig.ACTION_IMS_FEATURE_CHANGED)) {
                int phoneId = Utils.getMainCapabilityPhoneId();
