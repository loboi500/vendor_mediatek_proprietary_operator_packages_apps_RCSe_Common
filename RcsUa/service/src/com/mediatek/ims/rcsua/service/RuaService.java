...
import com.mediatek.rcs.provisioning.https.AcsService;
...
public final class RuaService extends Service {
    ...
    public IBinder onBind(Intent intent) {
        logger.debug("onBind with intent "+intent);
 ...
 */
    ...
    private void triggerRoiDeregistration() {
        logger.debug("triggerRoiDeregistration");
        adapter.sendRegDeregister();
    }

    private void triggerRoiRegistration() {
        logger.debug("triggerRoiRegistration");
        adapter.sendRegRegister();
    }

    private void triggerForceReregistration(Capability features) {
        logger.debug("triggerForceReregistration features: " + features + ",capability: " + capability);
        if (features == null)
            adapter.sendForceRegReregister(capability);
        else
            adapter.sendForceRegReregister(features);
    }
    ...
    void notifyAcsErrorReceived(int errorCode, String errorString) {
        logger.debug("notifyAcsErrorReceived");
        Message.obtain(imsCallbackHanlder, MSG_ACS_CONFIG_CHANGE_ERROR,
                       errorCode, 0, errorString).sendToTarget();
    }
   ...
   private void startAcsService() {
        logger.debug("startAcsService acsServiceConnection: "+acsServiceConnection
                     + ", acsService:" + (acsService != null));
        if (acsService != null)
            return;
        ...
        if (acsServiceConnection == null) {
            acsServiceConnection = new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    logger.debug("onServiceConnected ACS:" + service + ", count:" + mAcsRetryCount);
                    if (mAcsRetryCount > 0) mAcsRetryCount = 0;
                    try {
                        if (service != null) {
        ...
        new Runnable() {
            ...
            public void run() {
                ...
                startService(intent);
                boolean result = bindService(intent, acsServiceConnection, Context.BIND_AUTO_CREATE);
                UserManager manager = UserManager.get(Utils.getApplicationContext());
                boolean unlocked = manager != null ? manager.isUserUnlocked() : true;
                logger.info("bind acs:" + result + ", count:" + mAcsRetryCount
                            + ", unlocked:" + unlocked);
                if (!result && mAcsRetryCount < 600 && unlocked) {
                    mAcsRetryCount++;
                    imsCallbackHanlder.removeCallbacks(this);
                    imsCallbackHanlder.postDelayed(this, 1000);
                }
            }
        }.run();
    }
    ...
    private void registerCallbackToAcsService() {
        if (acsCallback == null) {
            acsCallback = new IAcsCallback.Stub() {
                ...
                @Override
                public void onAcsErrorReceived(int errorCode, String errorString) throws RemoteException {
                    notifyAcsErrorReceived(errorCode, errorString);
                }
            };
            try {
    ...
    private class RcsUaServiceImpl extends IRcsUaService.Stub {
        ...
        @Override
        public void triggerRoiDeregistration() throws RemoteException {
            RuaService.this.triggerRoiDeregistration();
        }

        @Override
        public void triggerRoiRegistration() throws RemoteException {
            RuaService.this.triggerRoiRegistration();
        }

        @Override
        public void triggerForceReregistration(Capability features) throws RemoteException {
            RuaService.this.triggerForceReregistration(features);
        }
        ...
        public void setAcsonfiguration(String rcsVersion, String rcsProfile, String clientVendor, String clientVersion) throws RemoteException {
            logger.debug("setAcsonfiguration rcsVersion: "+rcsVersion+" ,rcsProfile: "+rcsProfile+" ,clientVendor: "+clientVendor+" ,clientVersion: "+clientVersion);
            logger.debug("acsService value "+acsService);
            if (acsService != null) {
                acsService.setAcsonfiguration(rcsVersion,rcsProfile,clientVendor,clientVersion);
            }
        }
    ...
    private final Handler imsCallbackHanlder = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (MSG_ACS_CONFIG_CHANGE_ERROR == msg.what) {
                int errorCode = msg.arg1;
                String errorString =
                    (msg.obj != null && msg.obj instanceof String) ? (String)msg.obj : "";
                synchronized (acsCallbacks) {
                    final int N = acsCallbacks.beginBroadcast();
                    for (int i = 0; i < N; i++) {
                        try {
                            acsCallbacks.getBroadcastItem(i).onConfigErrorReceived(errorCode, errorString);
                        } catch (RemoteException e) {
                            // The RemoteCallbackList will take care of removing
                            // the dead object for us.
                        }
    ...
    private static final int MSG_ACS_CONFIG_CHANGE_ERROR = 2004;
