...
public final class Client {
    ...
    ISipEventCallback sipEventCallback = new ISipEventCallback.Stub() {
        ...
        @Override
        public void messageSent(String transactionId) throws RemoteException {
            for (SipChannel channel : activeChannels) {
                channel.handleSipMessageSent(transactionId);
            }
        }

        @Override
        public void messageSendFail(String transactionId, int reason) throws RemoteException {
            for (SipChannel channel : activeChannels) {
                channel.handleSipMessageSendFail(transactionId, reason);
            }
        }

        @Override
        public void availabilityChanged(boolean isAvailable) throws RemoteException {
            for (SipChannel channel : activeChannels) {
                channel.handleAvailability(isAvailable);
                break;
            }
        }
    };
