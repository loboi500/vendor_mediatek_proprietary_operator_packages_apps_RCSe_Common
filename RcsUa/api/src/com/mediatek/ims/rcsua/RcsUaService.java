...
public class RcsUaService {
    ...
    /**
     * Trigger Internet Deregistration.
     */
    public void triggerRoiDeregistration() {
        Log.d(TAG, "triggerRoiDeregistration");
        if (!isConnected()) {
            throw new IllegalStateException("RCS UA service disconnected");
        }

        try {
            serviceIntf.triggerRoiDeregistration();
        } catch (RemoteException e) {
    ...
    /**
     * Trigger Internet Registration.
     */
    public void triggerRoiRegistration() {
        Log.d(TAG, "triggerRoiRegistration");
        if (!isConnected()) {
            throw new IllegalStateException("RCS UA service disconnected");
        }

        try {
            serviceIntf.triggerRoiRegistration();
        } catch (RemoteException e) {
    ...
    /**
     * Forced trigger IMS registration for same feature sets
     * only worked when IMS already registered.
     */
    public void triggerForceReregistration() {
        ...
        try {
            serviceIntf.triggerForceReregistration(null);
        } catch (RemoteException e) {
    ...
    public Client registerClient(@NonNull ImsEventCallback callback) {
        ...
        try {
            Log.d(TAG, "registerClient:" + clientIntf + ",clientCount=" + clientCount);
    ...
    public Client[] getActiveClients() {
        ...
        Client client[] = new Client[len];
        for(int i=0; i<len; i++) {
            client[i] = clientList.get(i);
        }
        Log.d(TAG, "getActiveClients client:" + client);
        ...
        synchronized (lock) {
            if (acsCallbacks.isEmpty()) {
                acsEventCallback = new IAcsEventCallback.Stub() {
                    ...
                    @Override
                    public void onConfigErrorReceived(int errorCode, String errorString) {
                        Log.d(TAG, "onConfigErrorReceived errorCode:" + errorCode);
                        synchronized (lock) {
                            acsCallbacks.forEach(callback -> {
                                    callback.run(callback.new Runner(3, errorCode, errorString));
                                });
                        }
                    }
                };
    ...
    /**
     * Retrieve current ACS configuration for AOSP.
     *
     * @return ACS configuration data. null if ACS configuration not being valid at the time.
     */
    ...
    /**
     * Set Google ACS Configuration value.
     *
     * @param rcsVersion
     * @param rcsProfile
     * @param clientVendor
    ...
    public void setAcsonfiguration(String rcsVersion, String rcsProfile, String clientVendor, String clientVersion) {
        ...
        Log.d(TAG,"setAcsonfiguration !acsSupported: "+acsSupported);

        if (!acsSupported)
            throw new UnsupportedOperationException("ACS not supported");

        try {
            Log.e(TAG,"serviceIntf value "+serviceIntf);
            serviceIntf.setAcsonfiguration(rcsVersion,rcsProfile,clientVendor,clientVersion);
        } catch (RemoteException e) {
        }
    }
    ...
    public static boolean isAcsAvailable(@NonNull Context context) {
        ...
        try {
            /*Log.e(TAG,"pm.getPackageInfo("com.mediatek.rcs.provisioning", 0) value "+pm.getPackageInfo("com.mediatek.rcs.provisioning", 0));*/
    ...
    private RcsUaService(
        ...
        serviceConnection = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                if (mRetryCount > 0) mRetryCount = 0;
                synchronized (RcsUaService.class) {
                    if (INSTANCE == null)
                        return;
                }
        ...
        Log.e(TAG,"callerContext value "+callerContext);
        ...
        if (!result) {
            ...
            handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.i(TAG, "bindSerivce fail, retry:" + mRetryCount);
                        callerContext.startService(intent);
                        boolean res = callerContext.bindService(intent, serviceConnection,
                                                                Context.BIND_AUTO_CREATE);
                        if (!res && mRetryCount++ < 200) {
    ...
    private static volatile IRcsUaService serviceIntf;
