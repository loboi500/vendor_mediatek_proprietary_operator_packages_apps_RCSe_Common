...
public class PhoneUtils {
   ...
   public static Uri formatContactIdToUri(ContactId contactId) {
       ...
       //return Uri.parse(SIP_URI_HEADER + contactId + "@" + sRcsSettings.getUserProfileImsDomain()
         //      + ";user=phone");
           //TODO TAPI 1.6 see if tel or sip uri is needed
   }
    /**
     * M: resolve issue for error handling international prefix code, eg.
     * 0086xxx. @{
    ...
    public static String formatNumberToSipUri(String number) {
        ...
        /* SIP format with Display name, strip off Display name part */
        if (number.startsWith("\"")) {
            int index1 = number.indexOf("\"", 1);
            if (index1 > 0) {
                number = number.substring(index1+1);
                index1 = number.indexOf("sip:", 0);
                if (index1 > 0)
            ...
            if (index1 != -1) {
                number = number.substring(index1+1, number.indexOf(">", index1));
            }
        }
     ...
     * @return SIP URI
     */
    public static String formatUuidToSipUri(String uuid) {
        if (uuid == null) {
            return null;
        }
    ...
    public static String formatNumberToXDMSipUri(String number) {
        if (number == null) {
            return null;
        }
    ...
    /**
     * Extract user part phone number from a SIP-URI or Tel-URI or SIP address without "+" sign
     *
     * @param uri SIP or Tel URI
     * @return Number or null in case of error
     */
    public static String extractNumberFromUriWithoutPlus(String uri) {
        try {
            // Extract URI from address
            int index0 = uri.indexOf("<");
            if (index0 != -1) {
     ...
     * @return Number or null in case of error
     */
    public static String extractUuidFromUri(String uri) {
        if (uri == null) {
            return null;
        }
        ...
        try {
            ...
            if (index1 == -1) {
                index1 = uri.indexOf("tel:");
            }
            if (index1 != -1) {
                uri = uri.substring(index1+4, uri.length());
            }
    ...
    public static boolean compareNumbers(String contact1, String contact2) {
        ...
        String number1 = PhoneUtils.extractNumberFromUri(new String(contact1));
        String number2 = PhoneUtils.extractNumberFromUri(new String(contact2));
    ...
    public static boolean compareUuid(String contact1, String contact2) {
        if (contact1 == null || contact2 == null)
            return false;

        String uuid1 = PhoneUtils.extractUuidFromUri(new String(contact1));
        String uuid2 = PhoneUtils.extractUuidFromUri(new String(contact2));
    ...
    public static List<String> generateContactsStringList(List<ContactId> contacts) {
        List<String> contactsList = new ArrayList<String>();
        if (contacts != null && contacts.size() >= 1) {
            contactsList.add(contacts.get(0).toString());
            for (int i = 1; i < contacts.size(); i++) {
                ContactId element = contacts.get(i);
                contactsList.add(element.toString());
            }
        }
    ...
    public static String generateContactsText(List<String> contacts) {
        String result = null;
    ...
    public static List<String> generateContactsList(String contact) {
        String[] splited = null;
    ...
    public static boolean isUsingDummyAccount() {
        boolean usingDummyAccount =
                RcsSettings.getInstance().getAutoConfigMode() == RcsSettingsData.NO_AUTO_CONFIG ? true:false;
      //  Logger.d(TAG, "isUsingDummyAccount()-is using dummy account: " + usingDummyAccount);
        return usingDummyAccount;
    }

    public static boolean isSelfIdentity(String uri) {
        String id1 = RcsSettings.getInstance().readParameter(RcsSettingsData.PUBLIC_USER_IDENTITY_1);
        String id2 = RcsSettings.getInstance().readParameter(RcsSettingsData.PUBLIC_USER_IDENTITY_2);
        String id3 = RcsSettings.getInstance().readParameter(RcsSettingsData.PUBLIC_USER_IDENTITY_3);

        String id = extractUuidFromUri(new String(uri));

        return ((id1 != null)?id1.contains(id):false) ||
               ((id2 != null)?id2.contains(id):false) ||
               ((id3 != null)?id3.contains(id):false);
    }
    ...
    private static String getInternationalPrefix(String countryIso) {
        ...
        return Integer.toString(util.getCountryCodeForRegion(countryIso));
    }
    ...
    private static String getDefaultSimCountryIso() {
        ...
        iso = TelephonyManager.getDefault().getSimCountryIso(subId);
