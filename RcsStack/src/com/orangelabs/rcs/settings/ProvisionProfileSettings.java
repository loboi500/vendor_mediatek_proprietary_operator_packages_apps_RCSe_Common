...
public class ProvisionProfileSettings extends PreferenceActivity implements
    ...
    private static final String[] TMOBILE_ACCOUNTS = new String[] {
            ...
            "+8618228023995" };
    private static final String[] TMOBILE_DISPLAYNAMES = new String[] {
            "Liwen Chang", "Bruce Lee", "Mi Tang", "Meijia Li", "Yuanyuan Li",
            "Alexy Li", "Alexy Li1", "Elieen Yuan", "Elieen Yuan1",
            "Huan Feng", "Huan Feng1", "Free", "Fred", "Wenhuai", "Tesla",
            "Xun Han", "Xun Han1", "Zhanxiang Tang", "Zhanxiang Tang1" };
    private static final String[] VODAFONE_ACCOUNTS = new String[] {
            "+34667261567", "+34667265212", "+34667265241", "+34667256834",
            "+34600282222", };

    private static final String[] VODAFONE_DISPLAYNAMES = new String[] {
            "MTI1567", "MTI5212", "MTI5241", "MBJ6834", "MBJ2222" };
    public static final String[] ACCOUNTS = new String[] { "+34667261567",
            "+34667265212", "+34667265241", "+34667256834", "+34600282222",
            "+34670070026", "+34670070027", "+34670070028" };

    public static String[] sGSMNUMBERS = new String[] { "+919560699228",
            "+918586928441", "+918586928440", "+919711808988" };
    ...
    public static final String TELEFONICA_CSCF_ADDRESS
           = "pcscf.rcs.mnc007.mcc214.pub.3gppnetwork.org";
    /**
     * now,just use constant for the port, this value maybe changed.Now dummy
     * account use wifi network and tcp mode.
     */
    ...
    private static final String[] TELEFONICA_DISPLAYNAMES = new String[] {
            "Telefonica70071", "Telefonica70026", "Telefonica70027",
            "Telefonica70028" };
    /**
     * telefonica network .
     * use---------------------------------------------------------------------
     */
    private static final String[] ORANGE_ACCOUNTS = new String[] {
    ...
    static {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            account.mAutoConfigMode = "" + RcsSettingsData.NO_AUTO_CONFIG;
            account.mFtHttpServer =
                    "https://ftcontentserver.rcs.mnc001.mcc208.pub.3gppnetwork.org/rcse-hcs/upload";
        ...
        for (int i = 0; i < length; ++i) {
            ...
            account.mAccountPassword = VODAFONE_ACCOUNTS[i].substring(8,
                    VODAFONE_ACCOUNTS[i].length());
            ...
            if (SIP_OVER_TLS_ENABLED) {
              //  Logger.d(TAG,
              //          "SIPoTLS enabled, if use wifi then connect to tls server");
               ...
               // Logger.d(TAG,
               //         "SIPoTLS disabled, if use wifi then connect to tcp server");
      ...
      //      sAccountType = ACCOUNT_TYPE_MAP.get(VODAFONE_DOMAIN);
      //  } else {
         //   Logger.e(TAG, "static field: domain is not a valid value, but "
         //           + domain);
      //  }
    }
        ...
        if (EDIT_ACCOUNT.equals(preference.getKey())) {
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... params) {
                    RcseAccountConfiguration account = RCSE_ACCOUNT_MAP
                            .get(name);
                    if (account == null) {
                   ...
                   // Logger.v(TAG, "account:" + account.mAccountDisplayName
                   //         + "," + account.mDomain + ","
                   //         + account.mWifiSipProxyAddress + ","
                   //         + account.mWifiSipProxyPort + ","
                   //         + account.mMobileSipProxyAddress + ","
                   //         + account.mMobileSipProxyPort);
                    RcsSettings.getInstance().setFtHttpServer(
                            account.mFtHttpServer);
                    RcsSettings.getInstance().setFtHttpLogin(
                            account.mFtHttpLogin);
                    RcsSettings.getInstance().setFtHttpPassword(
                            account.mFtHttpPassword);
                    ...
                    RcsSettings.getInstance().setUserProfileImsDomain(
                            account.mDomain);
                    RcsSettings.getInstance().setUserProfileImsDisplayName(
                            account.mAccountDisplayName);

                    if (privateIdString != null
                            && privateIdString.startsWith("+34")) {
                        privateIdString = privateIdString.substring(1,
                                privateIdString.length());
                    }
                    RcsSettings.getInstance().setUserProfileImsPrivateId(
                            privateIdString);
                    RcsSettings.getInstance().setUserProfileImsPassword(
                            account.mAccountPassword);
                    RcsSettings.getInstance().setImsProxyAddrForMobile(
                            account.mMobileSipProxyAddress);
                    RcsSettings.getInstance().setImsProxyAddrForWifi(
                            account.mWifiSipProxyAddress);
                    RcsSettings.getInstance().setImConferenceUri(
                            account.mConferenceUri);
                    if (account.mMobileSipProxyPort != null) {
                        ...
                        RcsSettings.getInstance().writeParameter(
                                RcsSettingsData.SIP_DEFAULT_PORT,
                                account.mMobileSipProxyPort);
                    }
                    ...
                    RcsSettings.getInstance().writeParameter(
                            RcsSettingsData.AUTO_CONFIG_MODE,
