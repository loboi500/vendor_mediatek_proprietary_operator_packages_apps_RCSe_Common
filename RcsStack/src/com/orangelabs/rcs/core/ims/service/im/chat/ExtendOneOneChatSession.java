...
public class ExtendOneOneChatSession extends GroupChatSession {
     ...
     */
    public ExtendOneOneChatSession(ImsService parent, String conferenceId, OneOneChatSession oneoneSession, Map<String, ParticipantStatus> participants) {
    ...
    private SipRequest createInviteRequest(String content) throws SipException {
       ...
       /**
        ...
        if(RcsSettings.getInstance().isCPMSupported()){
            ...
            if(RichMessagingHistory.getInstance().getCoversationID(this.getContributionID(),2).equals("")) {
                // Set Call-Id
                ...
                if (logger.isActivated()) {
                    logger.info("CPMS createInviteRequest ExtendO2O:" + ConversationId);
                }
                setConversationID(ConversationId);
                RichMessagingHistory.getInstance().UpdateCoversationID(this.getRemoteContact(),ConversationId,1);
            }else {
                setConversationID(RichMessagingHistory.getInstance().getCoversationID(this.getRemoteContact(),1));
            }
