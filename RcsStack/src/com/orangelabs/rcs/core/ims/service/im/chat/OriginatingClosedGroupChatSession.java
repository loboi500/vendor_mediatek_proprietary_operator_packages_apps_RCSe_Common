 ...
 * @author jexa7410
 */
public class OriginatingClosedGroupChatSession extends GroupChatSession {
    /**
     * Boundary tag
     */
     ...
     */
    public OriginatingClosedGroupChatSession(ImsService parent, String conferenceId, String subject, Map<String, ParticipantStatus> participants) {
        ...
        if(RcsSettings.getInstance().isCPMSupported()){
            if (logger.isActivated()) {
                logger.info("CPMS OriginatingClosedGroupChatSession old call id: " + getDialogPath().getCallId());
            }
            ...
            if (rmHistory.getCoversationID(id,2).equals("")) {
                ...
                if (logger.isActivated()) {
                    logger.info("CPMS OriginatingClosedGroupChatSession callId: " + callId);
                }
            // Set conversation ID            
    ...
    public void run() {
        try {
            if (logger.isActivated()) {
                logger.info("Initiate a new closed group chat session as originating chatid: " + getSessionID());
            }
    ...
    private SipRequest createSipInvite() {
        logger.debug("createSipInvite()");
            ...
            if (logger.isActivated()){
            logger.debug("IP Address after format " + ipAddress);
        }
            ...
            String closedTag = "a=chatroom:closedgroupchat";
            if(RcsSettings.getInstance().isSupportOP07()){
                closedTag = "a=chatroom:org.3gpp.urn%3Aurn-7%3A3gpp-application.ims.iari.rcs.closedgroupchat";
            }
    ...
    private SipRequest createInviteRequest(String content) throws SipException {
        ...
        if(!RcsSettings.getInstance().isCPMSupported()){
            ...
            if (logger.isActivated()) {
                logger.info("CPMS OriginatingClosedGroupChatSession createInviteRequest 0");
            }
        ...
        if(RcsSettings.getInstance().isCPMSupported()){
            if (logger.isActivated()) {
                logger.info("CPMS OriginatingClosedGroupChatSession createInviteRequest 1");
            }
